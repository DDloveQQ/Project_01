...
public class MultiTcpServer {
    public static void main(String[] args) throws IOException {
        // 1、准备一个ServerSocket
        ServerSocket server = new ServerSocket(8888);  //绑定端口号8888
        ...
        // 2、监听一个客户端的连接
            ...
            ClientHandlerThread ct = new ClientHandlerThread(socket);  //建立一个客户端处理线程
            ct.start();
        }
        //这里没有关闭server，则永远监听
    }

    // 客户端处理线程(要继承Thread类,以此来实现多线程)
    static class ClientHandlerThread extends Thread {
        ...
        public void run() {
            try {
                ...
                //（4）反转
                    StringBuilder word = new StringBuilder(str);  //使用stringBuilder来处理string
                    word.reverse();
                //（5）返回给客户端
                    ps.println(word);
                }
